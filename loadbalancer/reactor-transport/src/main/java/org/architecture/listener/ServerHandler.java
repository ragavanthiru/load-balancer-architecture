...
import org.architecture.debug.ServerConnectionDebug;
import org.architecture.threads.WorkerPool;
import org.architecture.threads.WorkerPoolFactory;
import org.architecture.util.JavaUtils;
...
public class ServerHandler implements NHttpServerEventHandler {
    ...
    public ServerHandler(
        ...
        if (isMessageSizeValidationEnabled) {
            try {
                ...
                System.out.println("Invalid max message size configured for property \"valid.max.message.size.in.bytes\", "
    ...
    public void requestReceived(final NHttpServerConnection conn) {
        System.out.println("ServerHandler requestReceived");
...
//            }
    ...
    public void inputReady(final NHttpServerConnection conn, final ContentDecoder decoder) {
        System.out.println("ServerHandler inputReady");
        HttpContext context = conn.getContext();
        ...
        try {
            ...
            if (isMessageSizeValidationEnabled) {
                ...
                if (messageSizeSum > validMaxMessageSize) {
                    System.out.println("Payload exceeds valid payload size range, hence discontinuing chunk stream at "
...
//            }
    ...
    public void outputReady(final NHttpServerConnection conn, final ContentEncoder encoder) {
        System.out.println("ServerHandler outputReady");
        HttpContext context = conn.getContext();
    ...
    public void commitResponse(final NHttpServerConnection conn,
        ...
        try {
            ...
            System.out.println("Is Closed = "+conn.isResponseSubmitted());
    ...
    public void timeout(final NHttpServerConnection conn) {
        ...
        if (read == null || read) {
            if (log.isDebugEnabled()) {
                System.out.println(conn + ": Keepalive connection was closed");
            }
            shutdownConnection(conn, false, null);
        } else {
    ...
    public void connected(final NHttpServerConnection conn) {
        ...
        System.out.println("ServerHandler "+conn + ": New incoming connection");
//        }
        ...
        //if (log.isDebugEnabled()) {
            System.out.println(conn + ": Adding a connection : "
    ...
    public void responseReady(NHttpServerConnection conn) {
        System.out.println("ServerHandler responseReady1= "+JavaUtils.isTrueExplicitly(conn.getContext().getAttribute(
                NhttpConstants.FORCE_CLOSING)));
        System.out.println("ServerHandler responseReady2= "+!JavaUtils.isTrueExplicitly(conn.getContext().getAttribute(
                NhttpConstants.MESSAGE_IN_FLIGHT)));
        System.out.println("ServerHandler responseReady3= ");
        ...
        if (JavaUtils.isTrueExplicitly(conn.getContext().getAttribute(NhttpConstants.FORCE_CLOSING))
            ...
            try {
                //if (log.isDebugEnabled()) {
                System.out.println(conn + ": Closing a persisted connection since it is forced : " + conn);
                //}
                conn.close();
       ...
       // metrics.notifyReceivedMessageSize(conn.getMetrics().getReceivedBytesCount());
        ...
        //if (log.isTraceEnabled()) {
            //System.out.println(conn + ": Ready to send response");
        //}
    }
    ...
    public void markActiveConnectionsToBeClosed() {
        System.out.println("Marking the closing signal on the connection pool of size : "
    ...
    public void exception(NHttpServerConnection conn, Exception e) {
        ...
        if (e instanceof HttpException) {
            ...
            if (errMsg.indexOf("broken") != -1) {
                System.out.println("I/O error (Probably the connection " +
                        "was closed by the remote party):" + e.getMessage());
            } else {
                System.out.println("I/O error: " + e.getMessage()+ e);
            }
            /*if (metrics != null) {
                metrics.incrementFaultsReceiving();
            }*/
    ...
    public void exception(NHttpServerConnection conn, IOException e) {
        ...
        if (e instanceof ConnectionClosedException || (e.getMessage() != null &&
            ...
            System.out.println("Unexpected I/O error: " + e.getClass().getName()+ e);
            /*if (metrics != null) {
                metrics.incrementFaultsReceiving();
            }*/
    ...
    private void shutdownConnection(final NHttpServerConnection conn, boolean isError, String errorMsg) {
        ...
        synchronized (this) {
            if (!activeConnections.isEmpty() && activeConnections.remove(conn) /*&& log.isDebugEnabled()*/) {
                System.out.println("Removing the connection : " + conn
